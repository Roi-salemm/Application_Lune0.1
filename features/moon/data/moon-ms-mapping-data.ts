...
export async function fetchMsMappingNextNewMoonEventUtc(): Promise<{
  cycleStartUtc: Date | null;
  previousCycleStartUtc: Date | null;
  nextCycleStartUtc: Date | null;
}> {
  const db = await initMoonDb();
  const row = await db.getFirstAsync<{
    cycle_start_utc?: string | null;
    previous_cycle_start_utc?: string | null;
    next_cycle_start_utc?: string | null;
  }>(
    `WITH nm AS (
       SELECT datetime(phase_hour) AS nm_ts
       FROM ms_mapping
       WHERE phase = 0
     ...
     last_nm AS (
       SELECT nm_ts
       FROM nm
       WHERE nm_ts <= datetime('now')
       ORDER BY nm_ts DESC
       LIMIT 1
     )
     SELECT
       (SELECT nm_ts FROM last_nm) AS cycle_start_utc,
       (SELECT nm_ts
        FROM nm
        WHERE nm_ts < (SELECT nm_ts FROM last_nm)
        ORDER BY nm_ts DESC
        LIMIT 1) AS previous_cycle_start_utc,
       (SELECT nm_ts
        FROM nm
        WHERE nm_ts > datetime('now')
        ORDER BY nm_ts ASC
        LIMIT 1) AS next_cycle_start_utc`
  );

  return {
    cycleStartUtc: row?.cycle_start_utc ? parseSqlUtc(row.cycle_start_utc) : null,
    previousCycleStartUtc: row?.previous_cycle_start_utc ? parseSqlUtc(row.previous_cycle_start_utc) : null,
    nextCycleStartUtc: row?.next_cycle_start_utc ? parseSqlUtc(row.next_cycle_start_utc) : null,
  };
}
